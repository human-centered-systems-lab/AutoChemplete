...
def create_input_files(
    ...
    for impaths, smiles_sequences, split in [
        (train_image_paths, train_image_smiles, "TRAIN"),
        (val_image_paths, val_image_smiles, "VAL"),
    ]:
        if save_hdf5:
            with h5py.File(
                output_folder / f"{split}_IMAGES_{base_filename}.hdf5", "w"
            ) as h:
                    ...
                    try:
                        ...
                        for j, s in enumerate(smiles_sequence):
                            ...
                            enc_s = (
                                [token_map["<start>"]]
                                + [
                                    token_map.get(token, token_map["<unk>"])
                                    for token in s
                                ]
                                + [token_map["<end>"]]
                                + [token_map["<pad>"]] * (max_len - len(s))
                            )
                ...
                with open(
                    output_folder / f"{split}_SMILES_SEQUENCES_{base_filename}.json",
                    "w",
                ) as j:
                ...
                with open(
                    output_folder
                    / f"{split}_SMILES_SEQUENCE_LENS_{base_filename}.json",
                    "w",
                ) as j:
            ...
            images = (
                list()
            )  # h.create_dataset('images', (len(impaths), 3, 256, 256), dtype='uint8')
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                try:
                    ...
                    for j, s in enumerate(smiles_sequence):
                        ...
                        enc_s = (
                            [token_map["<start>"]]
                            + [token_map.get(token, token_map["<unk>"]) for token in s]
                            + [token_map["<end>"]]
                            + [token_map["<pad>"]] * (max_len - len(s))
                        )
                        # enc_s: list of integer
            ...
            # Save encoded sequences and their lengths to JSON files
            with open(
                output_folder / f"{split}_SMILES_SEQUENCES_{base_filename}.json", "w"
            ) as j:
                json.dump(enc_tokens, j)

            with open(
                output_folder / f"{split}_SMILES_SEQUENCE_LENS_{base_filename}.json",
                "w",
            ) as j:
                json.dump(sequence_lens, j)
