...
from utils import logger, load_reversed_token_map, str2bool, convert_smiles
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="train",
        help="choose work type 'train' or 'test'",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="image",
        help="choose which dataset, image or hdf5",
    )
    parser.add_argument(
        "--encoder_type",
        type=str,
    ...
    parser.add_argument(
        "--tf_encoder", type=int, default=0, help="the number of transformer layers"
    )
    parser.add_argument(
        "--tf_decoder",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="length of decoded SMILES sequence",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_load_path", type=str, default=None, help="model load path"
    )
    parser.add_argument(
        "--model_load_num", type=int, default=None, help="epoch number of saved model"
    )
    parser.add_argument(
    ...
    if config.work_type == "train":
        ...
        for itr in range(config.epochs):
            ...
            print(
                "t_loss: %s     t_accr: %s     v_loss: %s    v_accr: %s"
                % (t_l, t_a, v_l, v_a)
            )
        ...
        if not config.test_file_path == None:
            ...
            model.model_load()
            print("model loaded")
            submission = model.model_test(
                submission, data_list, reversed_token_map, transform
            )
        ...
        if not config.test_file_path == None:
            ...
            def _decode(models, _input):
                ...
                for model in models:
                    ...
                    predictions = model._decoder(
                        encoded_imgs,
                        decode_lengths=model._decode_length,
                        mode="generation",
                    )
            ...
            for i, dat in enumerate(data_list):
                ...
                if (
                    len(ms) == 0
                ):  # there is no decoded sequence that matches to SMILES format
                    print("decode fail")
                    fault_counter += 1
                    sequence = preds[0]
                    ...
                    len(ms) == 1
                ):  # there is only one decoded sequence that matches to SMILES format
                    ...
                    smiles_dict = calculate_similarity(
                        combination_of_smiles, combination_index
                    )
                    # sort the pairs by similarity score
                    smiles_dict = sorted(
                        smiles_dict.items(), key=(lambda x: x[1]), reverse=True
                    )

                    if (
                        smiles_dict[0][1] == 1.0
                    ):  # if a similar score is 1 we assume to those predictions are correct.
