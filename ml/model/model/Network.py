...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        if model_type == "wide_res":
            resnet = torchvision.models.wide_resnet101_2(
                pretrained=True
            )  # pretrained ImageNet wide_ResNet-101_2
        elif model_type == "res":
            resnet = torchvision.models.resnet152(
                pretrained=True
            )  # pretrained ImageNet wide_ResNet152
        elif model_type == "resnext":
            resnet = torchvision.models.resnext101_32x8d(
                pretrained=True
            )  # pretrained ImageNet wide_ResNet-101_2
        elif model_type == "efficientnetB0":
            resnet = torchvision.models.efficientnet_b0(
                pretrained=True
            )  # pretrained ImageNet efficientnet_b0
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, model_size, dropout=0.1, max_len=100):
        ...
        div_term = torch.exp(
            torch.arange(0, model_size, 2).float() * (-math.log(10000.0) / model_size)
        )
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        ), "For the Transformer, embed dim needs to be the same with decoder_rim"
        ...
        for _ in range(n_layers):
            ...
            # print(self.dropout)
            self.layers.append(
                TransformerDecoderLayer(self.decoder_dim, n_heads, self.dropout.p)
            )
    ...
    def predict(self, encoder_out, decode_lengths=70):
        ...
        for t in range(decode_lengths):
            ...
            self_attn_mask = torch.triu(
                encoded_caption_t.new_ones(
                    encoded_caption_t.size(1), encoded_caption_t.size(1)
                ),
                diagonal=1,
            ).bool()
